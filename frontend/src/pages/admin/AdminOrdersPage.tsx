  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Select value={status} onChange={(e) => { setStatus(e.target.value as OrderStatus | 'All'); setPage(1) }} className="w-44">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-ink-100">
                {data.items.map((o) => (
                  <tr key={o.id} className="cursor-pointer hover:bg-ink-50">
                    <td className="px-4 py-3"><Link to={`/admin/orders/${o.id}`} className="font-medium text-brand-600">#{o.id}</Link></td>
